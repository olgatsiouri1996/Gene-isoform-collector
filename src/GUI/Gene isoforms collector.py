...
# add gooey decorator
@Gooey(required_cols=3, program_name='Gene isoforms collector', header_bg_color= '#DCDCDC', terminal_font_color= '#DCDCDC', terminal_panel_color= '#DCDCDC')
def main():
    # Create the parser object
    ap = GooeyParser(description='Searches and exports the isoforms of each input gene loci')

    # Add arguments for the input files
    ap.add_argument('-loci','--loci file', required=True, widget='FileChooser', help='1-column text file containing gene loci')
    ap.add_argument('-input','--input fasta file', required=True, widget='FileChooser', help='input multi-FASTA file')
    ap.add_argument('-dir','--output directory', required=True, widget='DirChooser', help='Directory to save fasta files')

    # Add optional arguments
    ap.add_argument('-name','--multifasta filename',type=str,required=False, help='output single file name')
    ap.add_argument('-type','--type', required=False, type=str, widget='Dropdown', default='single file', choices=['single file','1 file per gene loci','1 file per gene isoform'], help="output type to choose")
    ...
    if args['type'] == 'single file':
        ...
        with open(args['multifasta filename']+'.fasta', 'w') as f:
            for locus, seq in seq_dict.items():
                f.write(f'>{locus}\n{seq}\n')
    elif args['type'] == '1 file per gene loci':
